...
[ -n "${COLMAP_BIN:-}" ] && [ -x "$COLMAP_BIN" ] || { echo "[loc] COLMAP_BIN not found. Set COLMAP_BIN=/path/to/colmap"; exit 1; }
NQ="${1:-80}"; STRIDE="${2:-4}"
...
"$COLMAP_BIN" feature_extractor --database_path "$DB" --image_path "$RGB" \
...
"$COLMAP_BIN" sequential_matcher --database_path "$DB" --FeatureMatching.use_gpu 0 \
  --SequentialMatching.overlap 30

# 2) reference 포즈 고정 시드(orbframe 좌표) → point_triangulator로 descriptor 모델
"$XRSPLAT_PYTHON" "$ROOT/scripts/make_orb_seed_from_db.py" --db "$DB" --orb "$REF" --out "$OUT/seed"
mkdir -p "$OUT/tri" "$OUT/registered"
"$COLMAP_BIN" point_triangulator --database_path "$DB" --image_path "$RGB" \
...
"$COLMAP_BIN" image_registrator --database_path "$DB" --input_path "$OUT/tri" --output_path "$OUT/registered" \
  --Mapper.ba_refine_focal_length 0 --Mapper.ba_refine_principal_point 0 --Mapper.ba_refine_extra_params 0
"$COLMAP_BIN" model_converter --input_path "$OUT/registered" --output_path "$OUT/registered" --output_type TXT
