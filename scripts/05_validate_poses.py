...
def validate_poses(scene, root=None, tol=0.01, stride=8, baseline=(0.2, 0.6),
                   overlap_gate=0.05, overlap_min=0.2, crop=0.6, pass_cm=3.0,
                   pairs=3, ts_min=float("-inf"), ts_max=float("inf")):
    """게이트 판정 → (passed: bool, metrics: dict).

    metrics["status"]: "pass" | "fail" | "cannot_validate"
    cannot_validate = 레코드/페어 부족 (default-PASS 절대 금지).
    """
    if root is None:
    ...
    kf_path = root / "outputs" / scene / "slam" / "KeyFrameTrajectory.txt"
    recs = []
    for ts, tx, ty, tz, qx, qy, qz, qw in sorted(load_tum(kf_path), key=lambda k: k[0]):
        if ts < ts_min or ts > ts_max:   # 동적(사람) 구간 키프레임 배제
            continue
        rel = match_nearest(ts, rgb, ts_keys, tol)
        if rel is None:
            continue
        name = Path(rel).name
        ...
        r = load_record(proc, name, (tx, ty, tz, qx, qy, qz, qw), imgs[name], assoc[name],
                        fx, fy, cx, cy, ds, stride)
        if r:
            recs.append(r)
    log.info("키프레임 레코드 %d개", len(recs))
    if len(recs) < 2:
        log.error("레코드 부족 → cannot validate")
        return False, {"status": "cannot_validate", "reason": "records < 2"}
    ...
    for i, j in combinations(range(len(recs)), 2):
        ...
        if not (baseline[0] <= bl <= baseline[1]):
            continue
        d = trees_twc[j].query(recs[i]["twc_world"], k=1)[0]
        ov = float(np.mean(d <= overlap_gate))
        if ov >= overlap_min:
    ...
    if not cands:
        log.error("co-visible + baseline 페어 없음 → CANNOT VALIDATE (default-PASS 금지)")
        return False, {"status": "cannot_validate", "reason": "co-visible 페어 없음"}

    outdir = root / "outputs" / scene / "validate"
    outdir.mkdir(parents=True, exist_ok=True)
    import open3d as o3d

    results = []
    for k, (ov, bl, i, j) in enumerate(cands[:pairs]):
        A, B = recs[i], recs[j]
        # 겹침 마스크 = A의 Twc점이 B의 Twc클라우드 게이트 내 (포즈버그 독립)
        dA = trees_twc[j].query(A["twc_world"], k=1)[0]
        m_full = dA <= overlap_gate
        u, v = A["pix"][:, 0], A["pix"][:, 1]
        cw, ch = crop, crop
        ...
        verdict = (m_center.sum() > 50) and (center_p50 <= pass_cm / 100.0)
    ...
    print(f"\n=> {'PASS ✅ (포즈 기하 정합)' if allpass else 'FAIL ❌'}  [pairs={len(results)}, 임계 p50<{pass_cm}cm]")
    if not allpass:
        print("   트리아지: 중앙크롭도 FAIL이면 배선(포즈↔이미지↔depth)/depth 의심. "
              "중앙 PASS·full FAIL이면 왜곡(가장자리). 오버레이 .ply로 ghost 방향 확인:", outdir)
    return allpass, {"status": "pass" if allpass else "fail", "results": results}
...
def main():
    ...
    passed, metrics = validate_poses(
        args.scene, root=args.root, tol=args.tol, stride=args.stride,
        baseline=args.baseline, overlap_gate=args.overlap_gate,
        overlap_min=args.overlap_min, crop=args.crop, pass_cm=args.pass_cm,
        pairs=args.pairs, ts_min=args.ts_min, ts_max=args.ts_max,
    )
