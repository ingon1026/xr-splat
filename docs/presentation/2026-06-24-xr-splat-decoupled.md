...
- 〔이미지: `docs/assets/reloc_demo_still_mcmc2m.png` / 영상: `reloc_replay_home.mp4`〕
...
| 실사 자산(M2 home, mcmc2m) | ✅ PSNR 27.96 / SSIM 0.871 / LPIPS 0.267, 프레임 유효 |
...
## Slide 13 — 품질 해소 & 다음 (結)
- ✅ **시각 품질 — MCMC strategy로 해소 (mcmc2m 채택)**
  - 기존 2M(refine-stop 5k 동결) 23.82 → mcmc2m **27.96 PSNR** (+4.1dB, LPIPS 0.39→0.27)
  - 병목은 raw-count가 아니라 배치/정제 — 같은 2M을 계속 정제하니 풀림
  - scene 타일링: 통제 비교(2M≈3M)로 불필요 확인 → **선반행**
- 다음: **ad-server 복귀** (motion 18개 smoke test)
- **연구 방향(분리가 맞다)은 결론남**
